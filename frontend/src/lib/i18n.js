export const translations = {
  fr: {
    ...
    outputs: {
      ...
      prepStartHint: 'Date à laquelle le Sprint 1 démarre — change-la si le travail a réellement commencé avant ou après aujourd\'hui. La roadmap, le Gantt, le burndown et le calendrier se recalculent à partir d\'elle. Le lancement visé, lui, est cette date + la durée totale des sprints ci-dessous — pour changer la durée sans perdre le travail déjà planifié, utilise le slider "Délai" de la carte Budget & Délai (Go-to-market), qui ne fait que déplacer la date cible ; le bouton "Reconstruire la roadmap" de cette même carte redimensionne aussi le nombre de sprints, si tu veux repartir sur de nouvelles bases.',
  ...
  en: {
    ...
    outputs: {
      ...
      prepStartHint: 'Date Sprint 1 begins — change it if work actually started before or after today. The roadmap, Gantt, burndown and calendar all recalculate from it. Projected launch is this date + the total sprint duration below — to change the duration without losing already-planned work, use the "Timeline" slider on the Budget & Timeline card (Go-to-market), which only moves the target date; that same card\'s "Rebuild the roadmap" button also resizes the sprint count if you want a fresh start.',
