// Filet de sécurité si l'IA échoue — mêmes gabarits que l'ancien générateur local,
// utilisés uniquement en repli (l'IA reste le chemin par défaut, cf. tableClient.js).
...
const TEMPLATES = [
  {
    ...
    rows: [['', 'Instagram', '', 'À contacter', ''], ['', 'TikTok', '', 'À contacter', ''], ['', 'YouTube', '', 'À contacter', '']]
  },
  {
...
export function generateTableFromPrompt(prompt) {
  ...
  return { title: prompt, columns: ['Nom', 'Statut', 'Notes'], rows: [['', '', ''], ['', '', ''], ['', '', '']] }
}
